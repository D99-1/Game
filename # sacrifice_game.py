...
class Player:
    ...
    def draw(self, surf):
        ...
        if self.shield_active:
            ...
            pygame.draw.circle(s, (255,255,255,180), (self.radius*2, self.radius*2), int(self.radius*2.3), 3)
...
def intro_screen():
    ...
    while True:
        ...
        if elapsed < 1000:  # first 1 sec fade-in
            alpha = int(255 * (elapsed / 1000))
        elif elapsed > 4000:  # last 1 sec fade-out
            alpha = int(255 * ((duration - elapsed) / 1000))
        else:
            alpha = 255
            ...
            for p in players:
                ...
                if e.type == pygame.KEYDOWN:
                    if e.key == p.keys["shield"] and p.hearts>0:
                        p.hearts -= 1; p.shield_active = True
        ...
        for e in enemies:
            for p in players:
                if p.alive and distance((e.x,e.y), p.pos()) < e.radius+p.radius:
                    if p.shield_active: p.shield_active=False
                    else: p.hearts -=1
...
# ---------- Run ----------
mode = menu_screen()
intro_screen()  # cinematic 5-second intro
game_loop(mode)
pygame.quit()
